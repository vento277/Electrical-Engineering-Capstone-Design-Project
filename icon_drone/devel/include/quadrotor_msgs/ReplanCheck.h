...
struct Printer< ::quadrotor_msgs::ReplanCheck_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::quadrotor_msgs::ReplanCheck_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "plan_points: ";
    if (v.plan_points.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.plan_points.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.plan_points[i]);
    }
    if (v.plan_points.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "check_points: ";
    if (v.check_points.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.check_points.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.check_points[i]);
    }
    if (v.check_points.empty() || false)
      s << "]";
    if (true || !indent.empty())
