  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::raspicam_node::MotionVectors_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.x.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.x[i]);
    }
    if (v.x.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.y.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.y[i]);
    }
    if (v.y.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.sad.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.sad[i]);
    }
    if (v.sad.empty() || true)
      s << "]";
  }
